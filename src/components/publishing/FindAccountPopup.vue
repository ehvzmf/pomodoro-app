<template>
  <BasePopup :is-open="isOpen" title="아이디/비밀번호 찾기" @close="closePopup">
    <div class="find-popup">
      ...
      <div class="popup-footer">
        <button class="btn btn-ghost" @click="closePopup">취소</button>
...
const confirmCode = () => {
  if(!form.code) { alert('인증번호를 입력하세요'); return }
  alert('인증번호 확인: '+form.code)
}

const onConfirm = () => { emit('confirm', { tab: activeTab.value, data: { ...form } }); closePopup() }
...
.find-popup .card {
  background: $white;
  padding: $spacing-md;
  border-radius: $radius-md; /* smaller radius */
  box-shadow: $shadow-md;
}

.tabs { display:flex; gap:$spacing-xs; margin-bottom:$spacing-sm }
.tab { @include button-reset; padding:$spacing-xs $spacing-sm; border-radius:$radius-sm; background:$gray-100 }
.tab.is-active { background:$white; box-shadow:$shadow-sm }

.tab-panel { max-height: 400px; overflow-y:auto; padding-right:$spacing-sm }
...
.popup-footer { position:sticky; bottom:0; display:flex; justify-content:flex-end; gap:$spacing-sm; margin-top:$spacing-sm; padding-top:$spacing-sm; background:linear-gradient(180deg, rgba(255,255,255,0), rgba(255,255,255,1)) }
